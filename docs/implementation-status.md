...
| Mobile plan configurator | Implemented mobile-first controls for time window, return margin, editable reachability-speed assumption, budget, travelers, transport, selectable vehicle profiles and manual hub; shows a text-only geometric estimate, with no map or GPS | [Screen](../src/app/page.tsx), [Styles](../src/app/page.module.css), [Specification](./design/plan-now.md) |
...
| Automated behavior tests | Vitest covers domain, application, storage and security behavior with synthetic fixtures; see the current test run rather than a duplicated count | [Tests](../src/domain/active-mission.test.ts), [Persistence tests](../src/adapters/local-state.test.ts) |
...
| Round-trip reachability estimate | Implemented as an explicitly approximate geometric radius derived from plan time, margin and editable assumed speed; its feasibility status remains `unknown`, regardless of route-evidence freshness | [Reachability domain](../src/domain/reachability.ts), [Plan integration](../src/application/plan-reachability.ts), [Tests](../src/domain/reachability.test.ts), [Plan tests](../src/application/plan-reachability.test.ts), [Feasibility contract](./data/feasibility.md) |
| Reachability map overlay contract | Implemented application serialization exposes approximate polygon, bounds and evidence status; it never returns a safe/tight result | [Map overlay](../src/application/reachability-map.ts), [Tests](../src/application/reachability-map.test.ts) |
| GPS, real map rendering, provider isochrones and route-backed return feasibility | Not implemented; the current estimate is text-only, and real trip feasibility remains unknown | [Feasibility target](./data/feasibility.md) |
...
- Runtime validation and the four feasibility states exist for focused domain
  slices. Provider freshness, schedules, route-based vehicle costing, map
  rendering and route-backed return evaluation remain unimplemented.
...
seguir una misión activa con checklist y progreso confirmado. El configurador
incorpora margen y velocidad supuesta editables y muestra un radio geométrico
orientativo que nunca eleva la viabilidad por encima de `unknown`. Incluye
...
regreso y la velocidad. Aún no existen mapa visual real, GPS, PWA offline ni
